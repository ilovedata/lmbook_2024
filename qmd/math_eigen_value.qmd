...
-   $\lambda$ 는 행렬 $\pmb A - \lambda \pmb I$ 의 행렬식이 0 이다.
...
이제 각각의 고유값에 대한 고유행렬을 다음과 같이 고유벡터의 정의에 의하여 구해보자. 
...
\end{bmatrix}
$$
유의할 점은 고유벡터는 방정식을 만족하는 무수히 많은 벡터 중에 하나의 예일 뿐이다. 예를 들어 길이가 1 인 단위벡터를 구하고 싶다면 위의 벡터를 길이가 1인 단위벡터로 바꾸면 된다. 
...
또한 $\lambda_1=2$ 에 대한 고유행렬은 다음과 같이 구할 수 있다.
...
-1 & 1
\end{bmatrix}
\quad \rightarrow \quad \pmb{P}^t \pmb{P} = \pmb{I}
$$
